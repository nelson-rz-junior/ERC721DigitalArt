...
class MyWallet extends React.Component {
        ...
        try {
            ...
            if (_total > 0) {
                ...
                if (_total <= 3) {
                    ...
                    rows = Math.ceil(_total / 3);
                }

                let columns = 3;
                let rowArr = Array.apply(null, {length: rows}).map(Number.call, Number);
    ...
    async submitArtSell() {
        try {
            ...
            await this.state.contractInstance.methods.resellArt(this.state.sellTokenId, priceInWei)
                .send({ from: this.state.user, gas: 6000000 });
