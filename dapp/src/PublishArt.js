...
class PublishArt extends React.Component {
    ...
    submitHandler = (event) => {
        ...
        if(this.isNotEmpty(title) && this.isNotEmpty(description) && this.isNotEmpty(authorName) && this.isNotEmpty(date) && 
            this.isNotEmpty(imageValue) && this.isNotEmpty(price))
        {
    ...
    async publishArt(title, description, date, authorName, price, imageValue) {
        try {
            await this.state.contractInstance.methods.createTokenAndSellArt(title, description, date, authorName, price, imageValue)
